...
public sealed class GcBuffer
{
    ...
    public GcBuffer(byte[] imageData, uint width, uint height, PixelFormat pixelFormat, uint pixelDynamicRangeMax, long frameID, ulong timeStamp)
    {
        ...
        // Validate array size.
        if (imageData.LongLength < width * height * GenICamConverter.GetBitsPerPixel(pixelFormat) / 8)
        ...
        Width = width;
        Height = height;
        NumChannels = GenICamConverter.GetNumChannels(pixelFormat);
        BitDepth = GenICamConverter.GetBitsPerPixelPerChannel(pixelFormat);
